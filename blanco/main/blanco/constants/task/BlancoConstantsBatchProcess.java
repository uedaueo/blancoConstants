...
public class BlancoConstantsBatchProcess {
    ...
    public static final void main(final String[] args) {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoConstantsBatchProcess: 入力パラメータ[" + arg + "]は無視されました。");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoConstantsBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
    ...
    public int process(final BlancoConstantsProcessInput input) throws IOException, IllegalArgumentException {
        ...
        // この箇所でコンパイルエラーが発生する場合、BlancoConstantsProcessインタフェースを実装して blanco.constants.taskパッケージに BlancoConstantsProcessImplクラスを作成することにより解決できる場合があります。
    ...
    public final int execute(final BlancoConstantsProcessInput input) throws IllegalArgumentException {
        try {
            ...
            System.out.println("BlancoConstantsBatchProcess: 入力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoConstantsBatchProcess: 入出力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoConstantsBatchProcess: ランタイム例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            ex.printStackTrace();
            // 異常終了。
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoConstantsBatchProcess: ランタイムエラーが発生しました。バッチ処理を中断します。:" + er.toString());
            er.printStackTrace();
            // 異常終了。
            return END_ERROR;
        }
    ...
    public void validateInput(final BlancoConstantsProcessInput input) throws IllegalArgumentException {
        ...
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoConstantsBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
        }
    }
}
